...
def get_two_actors_points(bodies_data):
    ...
    # if int(ske_name.split('_')[0]) == 13 and ske_name.split('_')[-2][0] == '2':  # DEBUG
    #     pass
...
def get_raw_denoised_data():
    ...
    for (idx, bodies_data) in enumerate(raw_skes_data):
        ...
        if num_bodies == 1:  # only 1 actor
            ...
            if bodies_data['num_frames'] >= noise_len_thres:
                if ske_name == 'violent-cam2-13_1745402645.999699_1.0_A010':
                    pass
                ...
                missing_skes_logger.info('%s\t%d\t%d' % (ske_name, bodies_data['num_frames'], num_bodies))
                global missing_count
                missing_count += 1
                continue
...
if __name__ == '__main__':
    ...
    root = "/work/cvcs2024/ViolenceDatasetHyperformer/FTD_processed"
    # root='./'
    statistics_path = root + "/statistics"
    skels_names = statistics_path + "/skes_available_name.txt"
    labels = statistics_path + "/label.txt"
    performers = statistics_path + "/performer.txt"
    setups = statistics_path + "/setup.txt"
    missing_skes = root + "/denoised_data/missing_skes.log"
    
    # read missing files names
    missing = []
    with open(missing_skes, 'r') as f:
        lines = f.readlines()
        for line in lines:
    ...
    # get files indexes
    indexes = []
    with open(skels_names, 'r') as f:
        lines = f.readlines()
        indexes = [lines.index(name) for name in lines if name.strip() in missing]
    ...
    # PROBLEMA : .pop non funziona come metodo perchè quando viene tolto elemento a certo indice tutto poi scala 

    with open(labels, 'r') as f:
        lines_label = f.readlines()
        #eliminate the line in the file with the index equal to the index of the missing file
    ...
    with open(skels_names, 'r') as f:
        ...
        for index in indexes:
            lines_names[index] = 'N'
    with open(performers, 'r') as f:
        lines_perform = f.readlines()
        for index in indexes:
            lines_perform[index] = 'N'
    with open(setups, 'r') as f:
        lines_setup = f.readlines()
        for index in indexes:
            lines_setup[index] = 'N'

    lines_label = [line for line in lines_label if line != 'N']
    lines_perform = [line for line in lines_perform if line != 'N']
    lines_setup = [line for line in lines_setup if line != 'N']
    lines_names = [line for line in lines_names if line != 'N']

    with open(labels, 'w') as f:
        f.write(''.join(lines_label))
    with open(performers, 'w') as f:
        f.write(''.join(lines_perform))
    with open(setups, 'w') as f:
        f.write(''.join(lines_setup))
    with open(skels_names, 'w') as f:
        f.write(''.join(lines_names))
