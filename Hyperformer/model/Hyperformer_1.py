...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=20, num_person=2, graph=None, graph_args=dict(), in_channels=3,
        ...
        self.fc = nn.Linear(2*24*num_of_heads, 10)
    ...
    def forward(self, x, y):
        ...
        x[0] = self.l10(x[0], self.joint_label, groups)

        # MODIFIED
        # caricamento dei .npz di yolo_wepon
        ...
        x[0] = x[0].view(N, M, C, -1)
        x[0] = x[0].mean(3).mean(1)
        
        # print(x[0].shape)

        # spatio temporal average pooling yolo 
        #x[1] = x[1].view(x[1].shape[0], x[1].shape[1], x[1].shape[2], -1)
        #x[1] = x[1].mean(3).mean(1)

        # print(x[1].shape)

        # hyperf:(batch_size, 216), yolo:(batch_size, 256)
        
        # max pool for alingment between yolo and hyperformer tensors
        #x[1] = F.adaptive_avg_pool1d(x[1].unsqueeze(1), 216).squeeze(1)

        # print(x[1].device)
        # print(x[0].shape, x[1].shape)

        # concat yolo and hyperformer
        final_x = torch.cat((x[0], x[1]), dim=1)

        # print(final_x.shape)

        final_x = self.drop_out(final_x)
