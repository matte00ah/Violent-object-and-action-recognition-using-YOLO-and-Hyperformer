...
# Hyperformer fine-tuned leaving unfrozen the last 3 layers other than the final linear one, and inserting YOLOWeapon information there
#Modified dropout from 0 to 0.1
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=20, num_person=2, graph=None, graph_args=dict(), in_channels=3,
                 drop_out=0.1, num_of_heads=9, joint_label=[], **kwargs):
        ...
        #self.l8 = TCN_ViT_unit(24*num_of_heads, 24*num_of_heads, A, residual=True, num_of_heads=num_of_heads, pe=True, num_point=num_point, layer=8)

        # MODIFIED: aggiungere qui modifiche dimensione input consinderando tensori yolo
        self.l8 = TCN_ViT_unit(2*24*num_of_heads, 24*num_of_heads, A, residual=True, stride=2, num_of_heads=num_of_heads, pe=True, num_point=num_point, layer=8)
        ...
        # standard ce loss
        self.fc = nn.Linear(24*num_of_heads, 10)
    ...
    def forward(self, x, y):
        ...
        N, C, T, V, M = x[0].size()
        # N, C, T, V, M = x.size()
        x[0] = x[0].permute(0, 4, 3, 1, 2).contiguous().view(N, M * V * C, T)
        x[0] = self.data_bn(x[0])

        ## n, c, t, v
        x[0] = x[0].view(N, M, V, C, T).contiguous().view(N * M, V, C, T).permute(0, 2, 3, 1)

        x[0] = self.l1(x[0], self.joint_label, groups)
        x[0] = self.l2(x[0], self.joint_label, groups)
        x[0] = self.l3(x[0], self.joint_label, groups)
        x[0] = self.l4(x[0], self.joint_label, groups)
        x[0] = self.l5(x[0], self.joint_label, groups)
        x[0] = self.l6(x[0], self.joint_label, groups)
        x[0] = self.l7(x[0], self.joint_label, groups)
        # MODIFIED
        #x[0] = self.l8(x[0], self.joint_label, groups)
        
        # MODIFIED
        # concatenazione hyperformer e yolo data
        # caricamento dati 
        for i in range(len(x[1])):
            x[1][i] = torch.tensor(np.load(x[1][i])['tensor'])
            x[1][i] = x[1][i].half()  # half precision
        
        tmp = torch.stack(x[1], dim=0)
        x[1] = tmp
        x[1] = x[1].cuda()
        # duplicazione dati yolo per fare match con prima dimensione di dati hyperformer di input a l8
        # hypefrormer data shape = x[0]: (128, 216, 32, 25)
        # expected yolo data shape = x[1]: (128, 216)
        x[1] = x[1].repeat_interleave(M, dim=0)
        # aggiunta dimensioni fittizie 
        x[1] = x[1].unsqueeze(-1).unsqueeze(-1)
        # matching delle dimensioni tra yolo e hyperformer
        x[1] = x[1].expand(-1, -1, x[0].shape[2], x[0].shape[3])
        
        # concatenazione yolo e hyperformer: expected input shape to l8 (128, 432, 32, 25)
        tmp = torch.cat((x[0], x[1]), dim=1)
        x = tmp
        ...
        # gestione delle dimensioni e il concat di yolo e scheletri
        
        # N*M, C, T, V
        _ , C, T, V = x.size()
        # spatial temporal average pooling skeletons
        x = x.view(N, M, C, -1)
        x = x.mean(3).mean(1)
