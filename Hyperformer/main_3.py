...
class Processor():
    ...
    def load_model(self):
        ...
        print(self.model)
        self.loss = nn.CrossEntropyLoss(label_smoothing=0.2).cuda(output_device) # MODIFIED: label smoothing added

        # FINE-TUNING
        # Freeze all layers except the last one and the final Linear layer (fc)
        unfreeze_layers = ['fc','l10'] 
        ...
        if self.arg.weights:
            ...
            # MODIFIED: aggiunto per saltare il caricamento dei pesi pre-addestrati per il layer l10
