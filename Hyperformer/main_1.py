...
class Processor():
    ...
    def load_model(self):
        ...
        # FINE-TUNING
        # Freeze all layers except the final Linear layer (fc)
        ...
        if self.arg.weights:
            ...
            # rimuovi pesi di strato lineare per poterlo riallenare con nuove dimensioni
            fc_keys = [key for key in weights.keys() if "fc" in key]
            for key in fc_keys:
...
if __name__ == '__main__':
    ...
    logger_lines['Setup'] = {
        ...
        "Test batch size": arg.test_batch_size,
        "Num Workers": arg.num_worker,
        "Optimizer": arg.optimizer,
        "Base LR": arg.base_lr,
        "LR decay": arg.lr_decay_rate,
        "Steps": arg.step,
        "Weight Decay": arg.weight_decay,
        "Seed": arg.seed,
        "Num epochs": arg.num_epoch
    }
    logger_lines['Results'] = []
    ...
    # hash = str(int(hashlib.md5(timestamp.encode()).hexdigest(), 16))[:4]
    with open(f"./NetworkResults-{job_id}-{timestamp}.json", 'w') as f:
        json.dump(logger_lines, f)