...
sys.path.append('/homes/mbulgarelli/Hyperformer/torchlight')
...
# MODIFIED: logger for better results formatting
results_logger = logging.getLogger('NetworkResults')
results_logger.setLevel(logging.INFO)
job_id = os.environ.get("SLURM_JOB_ID", "nojobid")
results_logger.addHandler(logging.FileHandler(f"./NetworkResults-{job_id}.json", mode='w'))
...
class Processor():
        ...
        if arg.phase == 'train':
            if not arg.train_feeder_args['debug']:
                ...
                if os.path.isdir(arg.model_saved_name):
                    print('log_dir: ', arg.model_saved_name, 'already exist: Removing')
    ...
    def load_data(self):
        ...
        if self.arg.phase == 'train':
            self.data_loader['train'] = torch.utils.data.DataLoader(
                ...
                shuffle=True,
                pin_memory=True,
                prefetch_factor=2, # MODIFIED originally 16
    ...
    def load_model(self):
        ...
        print(self.model)
        self.loss = nn.CrossEntropyLoss().cuda(output_device) # MODIFIED: label smoothing added

        # FINE-TUNING
        # Freeze all layers except the last 3 and final Linear layer (fc)
        unfreeze_layers = ['fc', 'l8', 'l9', 'l10'] 
        for name, param in self.model.named_parameters():
            if any(unfreeze in name for unfreeze in unfreeze_layers):  # Freeze all layers except those containing specified names
        ...
        if self.arg.weights:
            ...
            # MODIFIED: aggiunto per saltare il caricamento dei pesi pre-addestrati per i layer l8,l9,l10
            # MODIFIED: COMMENTATO PER FARE EVALUATION
    ...
    def load_optimizer(self):
        ...
        # MODIFIED: using ReduceLROnPlateau scheduler
        self.lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
            self.optimizer, mode='min', factor=self.arg.lr_decay_rate, patience=5, verbose=True
        )
    ...
    def train(self, epoch, save_model=False): 
        ...
        for batch_idx, (data, label, index) in enumerate(process):
            # MODIFIED: data è array con elemento 0 il tensore di skeleton e indice 1 l'indirizzo per il .npz di yolo
            self.global_step += 1
            with torch.no_grad():
                # MODIFIED  
                # data = data.float().cuda(self.output_device)
                data[0] = data[0].float().cuda(self.output_device)
        ...
        # MODIFIED: adding results to logger file
        results_logger.info(
            "{"
            f'"Training Epoch": {epoch + 1}, '
            f'"Loss": {np.mean(loss_value):.4f}, '
            f'"Loss2": {np.mean(loss_value2):.4f}, '
            f'"Accuracy": {np.mean(acc_value):.4f}, '
    ...
    def eval(self, epoch, save_score=False, loader_name=['test'], wrong_file=None, result_file=None):
        #MODIFIED: register layer hook for intermediate activation maps
        # layer = self.model.l10
        ...
        for ln in loader_name:
            ...
            if self.arg.phase == 'train':
                ...
                # MODIFIED: check is LR needs to be reduced
            ...
            #MODIFIED: add results to logger file
            results_logger.info(
                 "{"
    ...
    def start(self):
        if self.arg.phase == 'train':
            ...
            self.print_log(f'Dataset: {self.arg.train_feeder_args["data_path"]}')

            #MODIFIED: save results to logger file
            results_logger.info(
                 "],\n"
                 '"Summary":{'
                 f'"Best Accuracy": {self.best_acc:.4f}, '
                 f'"Best Epoch": {self.best_acc_epoch}, '
                 f'"Model Name": "{self.arg.work_dir}", '
                 f'"Total Parameters": {num_params}, '
                 f'"Weight Decay": {self.arg.weight_decay}, '
                 f'"Base Learning Rate": {self.arg.base_lr}, '
                 f'"Batch Size": {self.arg.batch_size}, '
                 f'"Test Batch Size": {self.arg.test_batch_size}, '
                 f'"Seed": {self.arg.seed}, '
                 f'"Dataset": "{self.arg.train_feeder_args["data_path"]}"'
                 "}\n"
             )

            #global logger_lines
            #logger_lines['Summary'] = {
                #"Best Accuracy": self.best_acc,
                #"Best Epoch": self.best_acc_epoch,
                #"Model Name": self.arg.work_dir,
                #"Total Parameters": num_params,
                #"Weight Decay": self.arg.weight_decay,
                #"Base Learning Rate": self.arg.base_lr,
                #"Batch Size": self.arg.batch_size,
                #"Test Batch Size": self.arg.test_batch_size,
                #"Seed": self.arg.seed
            #}
...
# MODIFIED: added to see input and output shapes of layer
...
if __name__ == '__main__':
    ...
    # MODIFIED: initializing results logger
    results_logger.info(
         '{\n'
         '"Setup":{\n'
         f'"Batch Size": {arg.batch_size},\n '
         f'"Num Workers": {arg.num_worker},\n '
         f'"Optimizer": "{arg.optimizer}",\n '
         f'"Base LR": {arg.base_lr},\n '
         f'"Weight Decay": {arg.weight_decay},\n '
         f'"Seed": {arg.seed},\n'
         f'"Dataset": "{arg.train_feeder_args["data_path"]}",\n'
         f'"Model": "{arg.work_dir}"\n'
         "},\n"
         '"Results":[\n'
     )
    ...
    processor.start()
    results_logger.info("\n}")  # Close the results list in the logger
